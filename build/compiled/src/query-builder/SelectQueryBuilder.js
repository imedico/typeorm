...
class SelectQueryBuilder extends QueryBuilder_1.QueryBuilder {
     ...
     * @param timestamp get datasets from this timestamp (temporal tables)
     */
    async getRawAndEntities(timestamp) {
        const queryRunner = this.obtainQueryRunner();
        let transactionStartedByUs = false;
        try {
            // start transaction if it was enabled
            ...
            const results = await this.executeEntitiesAndRawResults(queryRunner, timestamp);
            // close transaction if we started it
            if (transactionStartedByUs) {
                await queryRunner.commitTransaction();
            }
     ...
     * @param timestamp get datasets from this timestamp (temporal tables)
     */
    async executeEntitiesAndRawResults(queryRunner, timestamp) {
        if (!this.expressionMap.mainAlias)
     ...
     */
    async loadRawResults(queryRunner, timestamp = (0, date_fns_1.addYears)(new Date(), 10)) {
        this.setParameter("timestamp", DateUtils_1.DateUtils.mixedDateToUtcDatetimeString(timestamp));
        const [sql, parameters] = this.getQueryAndParameters();
