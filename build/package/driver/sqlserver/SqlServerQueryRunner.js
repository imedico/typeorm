...
class SqlServerQueryRunner extends BaseQueryRunner_1.BaseQueryRunner {
    ...
    createTableSql(table, createForeignKeys) {
        const columns = table.columns.map((column) => this.buildCreateColumnSql(table, column, false, true));
        if (table.versioning) {
            columns.push(`row_start DATETIME2 GENERATED ALWAYS AS ROW START HIDDEN NOT NULL`);
            columns.push(`row_end DATETIME2 GENERATED ALWAYS AS ROW END HIDDEN NOT NULL`);
    ...
    dropTableSql(tableOrName, ifExist) {
        ...
        query.push(`IF OBJECTPROPERTY(OBJECT_ID('${tablePath}'), 'TableTemporalType') = 2 ALTER TABLE ${tablePath} SET (SYSTEM_VERSIONING = OFF)`);
        query.push(`DROP TABLE ${ifExist ? "IF EXISTS " : ""}${tablePath}`);
        return new Query_1.Query(query.join("; "));
    }
    createViewSql(view) {
